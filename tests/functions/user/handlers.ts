...
describe('User handlers', () => {
  ...
  describe('register', () => {
    it('should return invalid email if email is invalid', async () => {
      ...
      expect(response.body.message).to.equal('Email is invalid')
    })
    ...
    it('should return invalid password if password is invalid', async () => {
      ...
      expect(response.status).to.equal(400)
      expect(response.body.message).to.equal('Password must be at least 8 characters long, contain at least one uppercase letter, one lowercase letter, one number and one special character')
    })
    ...
    it('should return invalid name if name is invalid', async () => {
      ...
      expect(response.status).to.equal(400)
      expect(response.body.message).to.equal('Name should contain only letters with fname and lname separated by a space')
    })
    ...
    it('should return invalid phone number if phone number is invalid', async () => {
      ...
      expect(response.status).to.equal(400)
      expect(response.body.message).to.equal('Phone number should be 10 digits long prefixed with +234 or 0')
    })
